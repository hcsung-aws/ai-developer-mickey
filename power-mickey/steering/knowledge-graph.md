...
1. **주 경로 (1홉)**: `GRAPH.md` Tags/Title 스캔 → Core 컬럼의 "언제" 힌트로 즉시 판단 → **Path 필드로 실경로 조회** → 해당 파일 상세.
   - 노드가 **`[ANCHOR]` (카테고리 앵커)이면** Path 가 가리키는 하위 그래프(`entries/{category}/GRAPH.md`)를 먼저 로딩 → 그 안에서 동일 규약으로 재탐색.
   - 노드가 **일반 entry 이면** Path 의 `entries/` 파일로 직행 (기존 flat 경로).
2. **관계 탐색 (2홉)**: entry 내부의 Links 로 연결 entry 이동. **Links 는 노드 ID만** — 실경로는 항상 GRAPH.md Path 필드로 조회 (파일 이동 시 Path 만 갱신되므로 링크 자동 유지). 3홉 이상은 원칙적으로 지양.
3. **트리거 매칭**: INDEX 트리거 키워드가 현재 작업과 매칭 시 entry 직접 로딩. 매칭 없으면 로딩하지 않고 상위 GRAPH 로 되돌아감. INDEX 의 Anchors § 에 등록된 카테고리는 하위 GRAPH 경유.

사용자가 "이전에 비슷한 거 했었나?" 요청 시 GRAPH.md 전체 스캔 (ANCHOR 노드 매칭 시 하위 GRAPH 포함).

### domain/ 계층 성장 (§20 연동)

`domain/entries/` 는 상한 없이 자연 성장하며, 트리거 기반 3단계로 점진 계층화한다. 상세 절차는 → §20 (Progressive Domain Hierarchy) 참조. **즉시 재편 강제 아님 — 트리거는 후보 제시(notify), 결정은 사용자 확인.**

- **entry 등록** (Curator 승격 시) → §20 Step 1: flat 배치 + GRAPH Path 컬럼 필수 + Links 는 노드 ID만.
- **단일 entry 하드 상한(400줄) 초과 감지 시** → §20 Step 2 (파일 분할) 후보 제시. 소프트 상한(200줄)은 감시만. **미초과 시** 조치 없음.
- **GRAPH.md 동일 태그 클러스터 7개 노드 이상 감지 시** → §20 Step 3 (카테고리 계층화) 후보 제시 — 파이프라인 고정 순서(§20) 준수, 계획 검증은 사용자 확인 필수. **미달 시** 조치 없음.
- 감지 시점은 `session-protocol.md` Continuing Step 1b 엔트로피 체크 (§3-6·7).
