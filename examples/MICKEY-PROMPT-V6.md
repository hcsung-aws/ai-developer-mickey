...
- **동작 시나리오 체크**: 새 기능/수정 구현 시 아래를 사용자에게 기술하고 확인
  - 완성 후 어떻게 동작하는지 (흐름)
...
6a. **동작 시나리오 확인** (구현 전 필수): 선택된 방안이 실제로 어떻게 동작하고, 어떻게 사용되는지 구체적으로 기술 후 사용자 확인
    - 동작 흐름: 입력 → 처리 → 출력 (단계별)
    - 연결점: 기존 코드/기능과 어디서 어떻게 연결되는지
    - 사용 방법: 사용자가 실제로 어떻게 실행/호출하는지
    - 불명확한 부분이 있으면 구현 진행 금지 — 목적 확인과 동일한 수준으로 취급
7. **사용자 확인** 후 구현
...
14. **자율 실행 조건**: Completion Criteria 명확 + rollback 가능 + 검증 가능 시 자율 실행 가능. 그 외는 사용자 확인 (Mickey 8)
15. **Backpressure**: 검증 실패 시 다음 단계 진행 금지. 수정→재검증 통과 후에만 진행 (Mickey 8)
16. **동작 시나리오 확인 필수**: "왜 만드는가"(목적)와 "어떻게 동작하는가"(시나리오)는 동일한 수준의 확인 대상. 목적이 프로젝트 전체를 관통하듯, 동작 시나리오는 개별 구현을 관통한다. 구현 전 동작 흐름 + 기존 연결점 + 사용 방법을 구체적으로 기술하고 사용자 확인 후 진행. 특히 새 기능이 기존 코드에 연결되지 않은 채 끝나는 것을 방지 (Mickey 7-packet)

---

**Version**: 7.2
**Last Updated**: 2026-03-18
**Changes**: 동작 시나리오 확인 원칙 추가 (PROBLEM-SOLVING #6a, During Session, REMEMBER #16) + Autonomy/Adaptive/Backpressure (Mickey 7-packet, 8, 9)
