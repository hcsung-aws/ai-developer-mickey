...
- **판단 지침**: 트리거된 클러스터가 응집된 도메인인가, 아니면 여러 도메인에 걸친 횡단 관점(aspect)인가를 사용자 확인 시 판단. aspect(예: verification/testing/distrust/architecture)면 skip. (제외 목록은 두지 않음 — 판단은 확인 시점에)
...
**Changes**: §20 Step 3 카테고리화 파이프라인 고정 순서 명문화 (M37): ① 트리거 notify → ② 연관 태그 합집합 실측으로 카테고리 경계 판단 → ③ 구성원 엄선(확실한 것만 이동, 애매한 것 flat 잔류) → ④ 계획 사용자 검증(생략 불가, 자의성 통제 장치) → ⑤ 분할 이동+그래프 구축(백업→이동→하위 GRAPH→ANCHOR→INDEX→링크 재검증).
**Prev Changes (v18)**: §20 Progressive Domain Hierarchy 신설 (Step 1/2/3, LINE 200/400, 클러스터 임계값 7, Path 컬럼 + 노드 ID 링크 정책). §3 엔트로피 체크에 domain 파일 크기 스캔(#6) + 카테고리 클러스터 스캔(#7) 추가. aspect/domain 판단은 제외 목록 대신 사용자 확인 시점 지침으로 (adaptive #6 준수). 기존 CURATOR-PROMPT "GRAPH.md 100줄" 문구 대체.
